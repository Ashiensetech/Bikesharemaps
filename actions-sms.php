...
function rent($number, $bike, $force = FALSE)
{
    ...
    if ($force == FALSE) {
        ...
        $result = $db->query("INSERT INTO activity_history SET user_id=$userId,bike_id=$bikeNum,action='force_rent',stand_id=$bikeStand");
        if ($currentUser) {
...
function returnBike($number, $bike, $stand, $message = "", $force = FALSE)
{
    ...
    $last_rented = $db->query("SELECT activity_time FROM `activity_history` WHERE user_id=$userId and bike_id=$bikeNum and action IN('rent','force_rent') ORDER BY id DESC LIMIT 1");
    ...
    if ($force == FALSE) {
        ...
        $result = $db->query("INSERT INTO activity_history SET user_id=$userId,bike_id=$bikeNum,action='force_return',stand_id=$standId,rental_time=$rental_time");
...
function freeBikes($number)
{
    ...
    $result = $db->query("SELECT count(bikeNum) as bikeCount,standDescription from bikes join stands on bikes.currentStand=stands.standId where stands.serviceTag=0 group by standDescription having bikeCount>0 order by standDescription");
    $rentedBikes = $result->num_rows;
    ...
    $listBikes = "";
    while ($row = $result->fetch_assoc()) {
        $listBikes .= $row["standDescription"] . ":" . $row["bikeCount"];
        $listBikes .= ",";
    }
    if ($rentedBikes > 1) $listBikes = substr($listBikes, 0, strlen($listBikes) - 1);

    $result = $db->query("SELECT count(bikeNum) as bikeCount,standDescription from bikes right join stands on bikes.currentStand=stands.standId where stands.serviceTag=0 group by standDescription having bikeCount=0 order by standDescription");
    $rentedBikes = $result->num_rows;
    ...
    while ($row = $result->fetch_assoc()) {
        $listBikes .= $row["standDescription"];
        $listBikes .= ",";
    }
    if ($rentedBikes > 1) $listBikes = substr($listBikes, 0, strlen($listBikes) - 1);
