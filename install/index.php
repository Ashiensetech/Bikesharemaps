...
$systemURL="https://".substr($systemURL,0,$installpos);
...
foreach ($stands as $stand)
   ...
   $result=$db->query("REPLACE INTO stands SET standName='".$stand."',serviceTag=0,standDescription='".$stand."'");
   }
for ($i=1;$i<=$_POST["bicyclestotal"];$i++)
   {
   $code=sprintf("%04d",rand(1000,9999)); //do not create a code with more than one leading zero or more than two leading 9s (kind of unusual/unsafe).
...
if ($connectors["sms"]):
?>
         <div class="form-group"><label for="standdescription-<?php echo $standid; ?>" class="control-label"><?php echo _('Place:'); ?></label> <input type="text" name="standdescription[<?php echo $standid; ?>]" id="standdescription-<?php echo $standid; ?>" class="form-control" value="<?php echo $row["standDescription"]; ?>" /></div>
<?php endif; ?>
...
foreach ($_POST["standdesc"] as $standid=>$value)
   {
   $result=$db->query("UPDATE stands SET standAddress='".$_POST["standdesc"][$standid]."',serviceTag='".$_POST["servicetag"][$standid]."',latitude='".$_POST["standlat"][$standid]."',longitude='".$_POST["standlong"][$standid]."' WHERE standId='$standid'");
   ...
   if (isset($_POST["standdescription"][$standid])) $result=$db->query("UPDATE stands SET standDescription='".$_POST["standdescription"][$standid]."' WHERE standId='$standid'");
   }
$db->conn->commit();
