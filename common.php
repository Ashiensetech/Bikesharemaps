...
function uploadVideo($file)
{
    ...
    $response = null;
    $tmpfile = $file["name"];
    //get file extension,size,length
    $ext = pathinfo($tmpfile, PATHINFO_EXTENSION);
    if (in_array($ext, array("mpg", "mp4", "avi", "mkv")))//extension valid
    {
        $filename = uniqid($tmpfile, TRUE) . "." . $ext;
        $filepath = $dir . $filename;
        if (move_uploaded_file($file["tmp_name"], $filepath))//file uploaded
        {
            $size = (filesize($filepath) / 1024) / 1024;
            $response = array("success", $filepath, $size);
        } else {
            $response = array($filepath);
        }
    } else {
...
function uploadImage($file)
{
    ...
    //get file extension
    $ext = pathinfo($tmpfile, PATHINFO_EXTENSION);
    if (in_array($ext, array("jpg", "png", "jpeg")))//extension valid
    {
        $filename = uniqid($tmpfile, TRUE) . "." . $ext;
        $filepath = $dir . $filename;
        if (move_uploaded_file($file["tmp_name"], $filepath))//file uploaded
        {
            $response = array("success", $filepath);
        } else {
            $response = "failed";
        }
    } else {
...
//            $mail->addAddress("workspaceinfotech@gmail.com");     // Add a recipient
////            $mail->addAddress("mahedi@workspaceit.com");     // Add a recipient
        }else{
...
function loggeduser()
{
    global $db;
    if (isset($_COOKIE["loguserid"]) AND isset($_COOKIE["logsession"])) {
        $userid = trim($_COOKIE["loguserid"]);
        $session = trim($_COOKIE["logsession"]);
        $result = $db->query("SELECT sessionId FROM sessions WHERE userId='$userid' AND sessionId='$session' AND timeStamp>'" . time() . "'");
        if ($result->num_rows == 1) return 1;
        else return 0;
    }
...
function checkeditablebikeno($bikeNum)
{
    global $db;
    $bikeNum = intval($bikeNum);
    $result = $db->query("SELECT bikeNum FROM bikes WHERE bike_num=$bikeNum");
    if (!$result->num_rows) {
        response('<h3>Bike ' . $bikeNum . ' does not exist!</h3>', ERROR);
    }
}

function checkeditableeventno($bikeNum)
{
    global $db;
    $bikeNum = intval($bikeNum);
    $result = $db->query("SELECT id FROM events WHERE event_num=$bikeNum");
    if (!$result->num_rows) {
        response('<h3>Bike ' . $bikeNum . ' does not exist!</h3>', ERROR);
    }
}
...
function broadcast($message, $notificationtype = 0)
{
    global $db, $systemname, $watches;

    $result = $db->query("SELECT number,mail FROM users");
    while ($row = $result->fetch_assoc()) {
        if ($notificationtype == 0) {
            sendSMS($row["number"], $message);
             sendEmail($watches["email"], $systemname . " " . _('notification'), $message);
...
function eventbroadcast($message, $eventid, $notificationtype = 0)
{
    global $db, $systemname, $watches;
    $result = $db->query("SELECT eu.user_id, u.number, u.mail FROM event_users eu 
...
function sendConfirmationEmail($emailto)
{
    ...
    global $db, $dbpassword, $systemname, $systemrules, $systemURL, $limits, $credit;
    ...
    $bikeLimit = $limits["registration"];
    $initCredit = $credit["init"];

    $db->query("INSERT INTO registration SET userKey='$userKey',userId='$userId'");
    $db->query("INSERT INTO limits SET userId='$userId',userLimit='$bikeLimit'");
    $db->query("INSERT INTO credit SET userId='$userId',credit='$initCredit'");
    ...
    $message = _('Hello') . ' ' . $firstname . ",\n\n" .
        ...
        _('System rules are available here:') . "\n" . $systemURL . $systemrules . "\n\n" .
        _('By clicking the following link you agree to the System rules(copy and paste in browser if not clickable):') . "\n" . $systemURL . "agree.php?key=" . $userKey;
...
function normalizephonenumber($number)
{
    ...
    if (substr($number, 0, 1) != $countrycode) $number = $countrycode . $number;
//    if (substr($number, 0, 3) <> $countrycode) $number = $countrycode . $number;
    return $number;
}

function generateRandomCode($length){
    $range_start = pow(10,$length - 1);
    $range_end = pow(10,$length) - 1;
    $code = sprintf("%04d", rand($range_start, $range_end));
    return $code;
}
